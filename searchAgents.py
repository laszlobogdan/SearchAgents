...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if problem.isGoalState(state):
        return 0
    currentPos = state[0]
    currentCorn = [state[1][0], state[1][1], state[1][2], state[1][3]]
    distance = [util.manhattanDistance(currentPos, corners[i]) for i in range(len(currentCorn)) if
                currentCorn[i] == 0]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    distance = [mazeDistance(position,food,problem.startingGameState) for food in foodGrid.asList()]
